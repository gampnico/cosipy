...
def init_snowpack(DATA):
    ...
    if initial_snowheight > 0.0:
        ...
        dT = (temperature_top - constants.temperature_bottom) / (
            initial_snowheight + constants.initial_glacier_height
        )
        if nlayers == 0:
        ...
        # add glacier
        nlayers = int(
            constants.initial_glacier_height
            / constants.initial_glacier_layer_heights
        )
        layer_heights = np.append(
            layer_heights,
            np.ones(nlayers) * constants.initial_glacier_layer_heights,
        )
        ...
        layer_T = np.append(
            layer_T,
            [
                layer_T[-1] - dT * constants.initial_glacier_layer_heights * i
                for i in range(1, nlayers + 1)
            ],
        )
        ...
        dT = (
            temperature_top - constants.temperature_bottom
        ) / constants.initial_glacier_height
        layer_heights = (
            np.ones(nlayers) * constants.initial_glacier_layer_heights
        )
        ...
        layer_T = np.array(
            [
                temperature_top
                - dT * constants.initial_glacier_layer_heights * i
                for i in range(1, nlayers + 1)
            ]
        )
        layer_liquid_water = np.zeros(nlayers)
...
def init_debris_pack(grid: Grid):
    ...
    The debris layers are inserted between the existing snow and ice
    layers.
    """
    debris_start = grid.get_number_snow_layers()
    temperature_ice_interface = np.minimum(  # icepack temperature
        grid.get_node_temperature(debris_start + 1), constants.zero_temperature
    )
    if constants.number_debris_layers > 0:
        debris_layer_thickness = (
            constants.debris_thickness / constants.number_debris_layers
        )
        for i in range(0, constants.number_debris_layers):
            grid.add_fresh_debris(
                debris_layer_thickness,
                constants.debris_density,
                temperature_ice_interface,
                0.0,
                debris_start,
            )
