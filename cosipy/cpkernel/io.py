...
class IOClass:
    ...
    def create_data_file(self):
        ...
        if cfg.restart:
            ...
            timestamp = pd.to_datetime(cfg.time_start).strftime(
                "%Y-%m-%dT%H-%M"
            )
            restart_path = os.path.join(
                cfg.data_path, "restart", "restart_" + timestamp + ".nc"
            )
            if os.path.isfile(restart_path) & (cfg.time_start != cfg.time_end):
                self.GRID_RESTART = xr.open_dataset(restart_path)
                self.restart_date = self.GRID_RESTART.time + np.timedelta64(
                    cn.dt, "s"
                ...
                print(  # if there is a problem kill the program
                    f"No restart file available for the given date {timestamp}\nOR start date {cfg.time_start} equals end date {cfg.time_end}\n"
                )
                # print('No restart file available for the given date %s' % (timestamp))
        ...
        if cfg.tile:
            if cfg.WRF:
                self.DATA = self.DATA.isel(
                    south_north=slice(cfg.ystart, cfg.yend),
                    west_east=slice(cfg.xstart, cfg.xend),
                )
            else:
                self.DATA = self.DATA.isel(
                    lat=slice(cfg.ystart, cfg.yend),
                    lon=slice(cfg.xstart, cfg.xend),
                )

        self.ny = self.DATA.dims[cfg.northing]
        self.nx = self.DATA.dims[cfg.easting]
        self.time = self.DATA.dims["time"]
    ...
    def init_data_dataset(self):
        ...
        self.DATA = xr.open_dataset(
            os.path.join(cfg.data_path, "input", cfg.input_netcdf)
        )
        ...
        if self.restart_date is None:
            # print('--------------------------------------------------------------')
            # print('\t Integration from %s to %s' % (cfg.time_start, cfg.time_end))
            # print('--------------------------------------------------------------\n')
            print(
                f"{'-'*62}\n\tIntegration from {cfg.time_start} to {cfg.time_end}\n{'-'*62}\n"
            )

            self.DATA = self.DATA.sel(
                time=slice(cfg.time_start, cfg.time_end)
            )  # Select dates from config.py
        else:
            # There is nothing to do if the dates are equal
            if self.restart_date == cfg.time_end:
                print("Start date equals end date ... no new data ... EXIT")
                sys.exit(1)
            else:
                # otherwise, run the model from the restart date to the defined end date
                # print('Starting from %s (from restart file) to %s (from config.py) \n' % (self.restart_date.values, cfg.time_end))
                print(
                    f"Starting from {self.restart_date.values} to {cfg.time_end} (from config.py)\n"
                )

                self.DATA = self.DATA.sel(
                    time=slice(self.restart_date, cfg.time_end)
                )

        if cfg.time_start < start_interval:
            print(
                "\n WARNING! Selected startpoint before first timestep of input data\n"
            )
        if cfg.time_end > end_interval:
            print(
                "\n WARNING! Selected endpoint after last timestep of input data\n"
            )
        if cfg.time_start > end_interval or cfg.time_end < start_interval:
        ...
        def check(field, max, min):
            ...
            if np.nanmax(field) > max or np.nanmin(field) < min:
                print(
                    "Please check the input data, its seems they are out of range %s MAX: %.2f MIN: %.2f \n"
                    % (
                        str.capitalize(field.name),
                        np.nanmax(field),
                        np.nanmin(field),
                    )
                )
    ...
    def init_result_dataset(self):
        ...
        self.RESULT.attrs["Start_from_restart_file"] = str(cfg.restart)
        self.RESULT.attrs["Stake_evaluation"] = str(cfg.stake_evaluation)
        self.RESULT.attrs["WRF_simulation"] = str(cfg.WRF)
        self.RESULT.attrs["Compression_level"] = cfg.compression_level
        self.RESULT.attrs["Slurm_use"] = str(cfg.slurm_use)
        self.RESULT.attrs["Full_fiels"] = str(cfg.full_field)
        self.RESULT.attrs["Force_use_TP"] = str(cfg.force_use_TP)
        self.RESULT.attrs["Force_use_N"] = str(cfg.force_use_N)
        self.RESULT.attrs["Tile_of_glacier_of_interest"] = str(cfg.tile)

        # Global attributes from constants.py
        self.RESULT.attrs["Time_step_input_file_seconds"] = cn.dt
        self.RESULT.attrs["Max_layers"] = cn.max_layers
        self.RESULT.attrs["Z_measurment_height"] = cn.z
        self.RESULT.attrs["Stability_correction"] = cn.stability_correction
        self.RESULT.attrs["Albedo_method"] = cn.albedo_method
        self.RESULT.attrs["Densification_method"] = cn.densification_method
        self.RESULT.attrs["Penetrating_method"] = cn.penetrating_method
        self.RESULT.attrs["Roughness_method"] = cn.roughness_method
        self.RESULT.attrs["Saturation_water_vapour_method"] = (
            cn.saturation_water_vapour_method
        )

        self.RESULT.attrs["Initial_snowheight"] = (
            cn.initial_snowheight_constant
        )
        self.RESULT.attrs["Initial_snow_layer_heights"] = (
            cn.initial_snow_layer_heights
        )
        self.RESULT.attrs["Initial_glacier_height"] = cn.initial_glacier_height
        self.RESULT.attrs["Initial_glacier_layer_heights"] = (
            cn.initial_glacier_layer_heights
        )
        self.RESULT.attrs["Initial_top_density_snowpack"] = (
            cn.initial_top_density_snowpack
        )
        self.RESULT.attrs["Initial_bottom_density_snowpack"] = (
            cn.initial_bottom_density_snowpack
        )
        self.RESULT.attrs["Temperature_bottom"] = cn.temperature_bottom
        self.RESULT.attrs["Const_init_temp"] = cn.const_init_temp

        self.RESULT.attrs["Center_snow_transfer_function"] = (
            cn.center_snow_transfer_function
        )
        self.RESULT.attrs["Spread_snow_transfer_function"] = (
            cn.spread_snow_transfer_function
        )
        self.RESULT.attrs["Multiplication_factor_for_RRR_or_SNOWFALL"] = (
            cn.mult_factor_RRR
        )
        self.RESULT.attrs["Minimum_snow_layer_height"] = (
            cn.minimum_snow_layer_height
        )
        self.RESULT.attrs["Minimum_snowfall"] = cn.minimum_snowfall

        self.RESULT.attrs["Remesh_method"] = cn.remesh_method
        self.RESULT.attrs["First_layer_height_log_profile"] = (
            cn.first_layer_height
        )
        self.RESULT.attrs["Layer_stretching_log_profile"] = cn.layer_stretching

        self.RESULT.attrs["Merge_max"] = cn.merge_max
        self.RESULT.attrs["Layer_stretching_log_profile"] = cn.layer_stretching
        self.RESULT.attrs["Density_threshold_merging"] = (
            cn.density_threshold_merging
        )
        self.RESULT.attrs["Temperature_threshold_merging"] = (
            cn.temperature_threshold_merging
        )

        self.RESULT.attrs["Density_fresh_snow"] = cn.constant_density
        self.RESULT.attrs["Albedo_fresh_snow"] = cn.albedo_fresh_snow
        self.RESULT.attrs["Albedo_firn"] = cn.albedo_firn
        self.RESULT.attrs["Albedo_ice"] = cn.albedo_ice
        self.RESULT.attrs["Albedo_mod_snow_aging"] = cn.albedo_mod_snow_aging
        self.RESULT.attrs["Albedo_mod_snow_depth"] = cn.albedo_mod_snow_depth
        self.RESULT.attrs["Roughness_fresh_snow"] = cn.roughness_fresh_snow
        self.RESULT.attrs["Roughness_ice"] = cn.roughness_ice
        self.RESULT.attrs["Roughness_firn"] = cn.roughness_firn
        self.RESULT.attrs["Aging_factor_roughness"] = cn.aging_factor_roughness
        self.RESULT.attrs["Snow_ice_threshold"] = cn.snow_ice_threshold

        self.RESULT.attrs["lat_heat_melting"] = cn.lat_heat_melting
        self.RESULT.attrs["lat_heat_vaporize"] = cn.lat_heat_vaporize
        self.RESULT.attrs["lat_heat_sublimation"] = cn.lat_heat_sublimation
        self.RESULT.attrs["spec_heat_air"] = cn.spec_heat_air
        self.RESULT.attrs["spec_heat_ice"] = cn.spec_heat_ice
        self.RESULT.attrs["spec_heat_water"] = cn.spec_heat_water
        self.RESULT.attrs["k_i"] = cn.k_i
        self.RESULT.attrs["k_w"] = cn.k_w
        self.RESULT.attrs["k_a"] = cn.k_a
        self.RESULT.attrs["water_density"] = cn.water_density
        self.RESULT.attrs["ice_density"] = cn.ice_density
        self.RESULT.attrs["air_density"] = cn.air_density
        self.RESULT.attrs["sigma"] = cn.sigma
        self.RESULT.attrs["zero_temperature"] = cn.zero_temperature
        self.RESULT.attrs["Surface_emission_coeff"] = cn.surface_emission_coeff

        # extra attrs
        self.RESULT.attrs["surface_emission_coeff_debris"] = (
            cn.surface_emission_coeff_debris
        )
        self.RESULT.attrs["critical_snowpack_thickness"] = (
            cn.critical_snowpack_thickness
        )
        self.RESULT.attrs["lejeune_weighting_coefficient"] = (
            cn.lejeune_weighting_coefficient
        )
        self.RESULT.attrs["number_debris_layers"] = cn.number_debris_layers
        self.RESULT.attrs["debris_thickness"] = cn.debris_thickness
        self.RESULT.attrs["debris_max_temperature"] = cn.debris_max_temperature
        self.RESULT.attrs["roughness_debris"] = cn.roughness_debris
        self.RESULT.attrs["albedo_debris"] = cn.albedo_debris
        self.RESULT.attrs["debris_density"] = cn.debris_density
        self.RESULT.attrs["spec_heat_debris"] = cn.spec_heat_debris
        self.RESULT.attrs["debris_structure"] = cn.debris_structure
        self.RESULT.attrs["debris_porosity"] = cn.debris_porosity
        self.RESULT.attrs["debris_void_porosity"] = cn.debris_void_porosity
        self.RESULT.attrs["debris_void_density"] = cn.debris_void_density
        self.RESULT.attrs["debris_packing_porosity"] = (
            cn.debris_packing_porosity
        )
        self.RESULT.attrs["thermal_conductivity_debris"] = (
            cn.thermal_conductivity_debris
        )
        self.RESULT.attrs["thermal_diffusivity_debris"] = (
            cn.thermal_diffusivity_debris
        )

        # Variables given by the input dataset
        self.add_variable_along_latlon(
            self.RESULT, self.DATA.HGT, "HGT", "m", "Elevation"
        )
        self.add_variable_along_latlon(
            self.RESULT, self.DATA.MASK, "MASK", "boolean", "Glacier mask"
        )
        ...
        self.add_variable_along_latlontime(
            self.RESULT, self.DATA.T2, "T2", "K", "Air temperature at 2 m"
        )
        self.add_variable_along_latlontime(
            self.RESULT, self.DATA.RH2, "RH2", "%", "Relative humidity at 2 m"
        )
        self.add_variable_along_latlontime(
            self.RESULT,
            self.DATA.U2,
        ...
        self.add_variable_along_latlontime(
            self.RESULT, self.DATA.PRES, "PRES", "hPa", "Atmospheric pressure"
        )
        self.add_variable_along_latlontime(
            self.RESULT,
            self.DATA.G,
        ...
        if "RRR" in self.DATA:
            self.add_variable_along_latlontime(
                self.RESULT, self.DATA.RRR, "RRR", "mm", "Total precipiation"
            )
        else:
            self.add_variable_along_latlontime(
                self.RESULT,
        ...
        if "SNOWFALL" in self.DATA:
            self.add_variable_along_latlontime(
                self.RESULT, self.DATA.SNOWFALL, "SNOWFALL", "m", "Snowfall"
            )

        if "N" in self.DATA:
            self.add_variable_along_latlontime(
                self.RESULT, self.DATA.N, "N", "-", "Cloud fraction"
            )
        else:
            self.add_variable_along_latlontime(
                self.RESULT,
    ...
    def create_global_result_arrays(self):
        ...
        # extra sfc_vars
        self.QPS = np.full((self.time, self.ny, self.nx), np.nan)
        self.SWnet = np.full((self.time, self.ny, self.nx), np.nan)

        if cfg.full_field:
            if "HEIGHT" in self.full:
                self.LAYER_HEIGHT = np.full(
                    (self.time, self.ny, self.nx, cn.max_layers), np.nan
                )
            if "RHO" in self.full:
                self.LAYER_RHO = np.full(
                    (self.time, self.ny, self.nx, cn.max_layers), np.nan
                )
            if "T" in self.full:
                self.LAYER_T = np.full(
                    (self.time, self.ny, self.nx, cn.max_layers), np.nan
                )
            if "LWC" in self.full:
                self.LAYER_LWC = np.full(
                    (self.time, self.ny, self.nx, cn.max_layers), np.nan
                )
            if "CC" in self.full:
                self.LAYER_CC = np.full(
                    (self.time, self.ny, self.nx, cn.max_layers), np.nan
                )
            if "POROSITY" in self.full:
                self.LAYER_POROSITY = np.full(
                    (self.time, self.ny, self.nx, cn.max_layers), np.nan
                )
            if "ICE_FRACTION" in self.full:
                self.LAYER_ICE_FRACTION = np.full(
                    (self.time, self.ny, self.nx, cn.max_layers), np.nan
                )
            if "IRREDUCIBLE_WATER" in self.full:
                self.LAYER_IRREDUCIBLE_WATER = np.full(
                    (self.time, self.ny, self.nx, cn.max_layers), np.nan
                )
            if "REFREEZE" in self.full:
                self.LAYER_REFREEZE = np.full(
                    (self.time, self.ny, self.nx, cn.max_layers), np.nan
                )
            self.LAYER_THERMAL_CONDUCTIVITY = np.full(
                (self.time, self.ny, self.nx, cn.max_layers), np.nan
            )
            self.LAYER_NTYPE = np.full(
                (self.time, self.ny, self.nx, cn.max_layers), np.nan
            )
            self.LAYER_G_PENETRATING = np.full(
                (self.time, self.ny, self.nx, cn.max_layers), np.nan
            )
    ...
    def copy_local_to_global(
        ...
        local_LAYER_LWC,
        local_LAYER_CC,
        local_LAYER_POROSITY,
        local_LAYER_ICE_FRACTION,
        local_LAYER_IRREDUCIBLE_WATER,
        local_LAYER_REFREEZE,
        # extra sfc_vars
        local_QPS,
        local_SWnet,
        # extra layer_vars
        local_LAYER_THERMAL_CONDUCTIVITY,
        local_LAYER_NTYPE,
        local_LAYER_G_PENETRATING,
    ):
        ...
        if cfg.full_field:
            ...
            if "IRREDUCIBLE_WATER" in self.full:
                self.LAYER_IRREDUCIBLE_WATER[:, y, x, :] = (
                    local_LAYER_IRREDUCIBLE_WATER
                )
            if "REFREEZE" in self.full:
                self.LAYER_REFREEZE[:, y, x, :] = local_LAYER_REFREEZE
            # extra layer_vars
            self.LAYER_THERMAL_CONDUCTIVITY[:, y, x, :] = (
                local_LAYER_THERMAL_CONDUCTIVITY
            )
            self.LAYER_NTYPE[:, y, x, :] = local_LAYER_NTYPE
            self.LAYER_G_PENETRATING[:, y, x, :] = local_LAYER_G_PENETRATING
    ...
    def write_results_to_file(self):
        if "RAIN" in self.atm:
            self.add_variable_along_latlontime(
                self.RESULT, self.RAIN, "RAIN", "mm", "Liquid precipitation"
            )
        if "SNOWFALL" in self.atm:
            self.add_variable_along_latlontime(
                self.RESULT, self.SNOWFALL, "SNOWFALL", "m w.e.", "Snowfall"
            )
        if "LWin" in self.atm:
            self.add_variable_along_latlontime(
                self.RESULT,
        ...
        if "H" in self.atm:
            self.add_variable_along_latlontime(
                self.RESULT, self.H, "H", "W m\u207b\xb2", "Sensible heat flux"
            )
        if "LE" in self.atm:
            self.add_variable_along_latlontime(
                self.RESULT, self.LE, "LE", "W m\u207b\xb2", "Latent heat flux"
            )
        if "B" in self.atm:
            self.add_variable_along_latlontime(
                self.RESULT, self.B, "B", "W m\u207b\xb2", "Ground heat flux"
            )
        if "QRR" in self.atm:
            self.add_variable_along_latlontime(
                self.RESULT, self.QRR, "QRR", "W m\u207b\xb2", "Rain heat flux"
            )
        if "surfMB" in self.internal:
            self.add_variable_along_latlontime(
                self.RESULT,
        ...
        if "MB" in self.internal:
            self.add_variable_along_latlontime(
                self.RESULT, self.MB, "MB", "m w.e.", "Mass balance"
            )
        if "Q" in self.internal:
            self.add_variable_along_latlontime(
                self.RESULT, self.Q, "Q", "m w.e.", "Runoff"
            )
        if "SNOWHEIGHT" in self.internal:
            self.add_variable_along_latlontime(
                self.RESULT, self.SNOWHEIGHT, "SNOWHEIGHT", "m", "Snowheight"
            )
        if "TOTALHEIGHT" in self.internal:
            self.add_variable_along_latlontime(
                self.RESULT,
        ...
        if "TS" in self.atm:
            self.add_variable_along_latlontime(
                self.RESULT, self.TS, "TS", "K", "Surface temperature"
            )
        if "ALBEDO" in self.atm:
            self.add_variable_along_latlontime(
                self.RESULT, self.ALBEDO, "ALBEDO", "-", "Albedo"
            )
        if "LAYERS" in self.internal:
            self.add_variable_along_latlontime(
                self.RESULT, self.LAYERS, "LAYERS", "-", "Number of layers"
            )
        if "ME" in self.internal:
            self.add_variable_along_latlontime(
                self.RESULT,
        ...
        if "REFREEZE" in self.internal:
            self.add_variable_along_latlontime(
                self.RESULT, self.REFREEZE, "REFREEZE", "m w.e.", "Refreezing"
            )
        if "subM" in self.internal:
            self.add_variable_along_latlontime(
                self.RESULT, self.subM, "subM", "m w.e.", "Subsurface melt"
            )
        if "Z0" in self.atm:
            self.add_variable_along_latlontime(
                self.RESULT, self.Z0, "Z0", "m", "Roughness length"
            )
        if "surfM" in self.internal:
            self.add_variable_along_latlontime(
                self.RESULT, self.surfM, "surfM", "m w.e.", "Surface melt"
            )
        if "MOL" in self.internal:
            self.add_variable_along_latlontime(
                self.RESULT, self.MOL, "MOL", "", "Monin Obukhov length"
            )
        ...
        if cfg.full_field:
            ...
            self.add_variable_along_latlonlayertime(
                self.RESULT,
                self.LAYER_THERMAL_CONDUCTIVITY,
                "LAYER_THERMAL_CONDUCTIVITY",
                "W m^-1 K^-1",
                "Thermal conductivity",
            )
            self.add_variable_along_latlonlayertime(
                self.RESULT, self.LAYER_NTYPE, "LAYER_NTYPE", "-", "Node type"
            )
            self.add_variable_along_latlonlayertime(
                self.RESULT,
                self.LAYER_G_PENETRATING,
                "LAYER_G_PENETRATING",
                "W m\u207b\xb2^-2",
    ...
    def create_empty_restart(self) -> xr.Dataset:
        ...
        dataset.coords["layer"] = np.arange(cn.max_layers)
    ...
    def create_global_restart_arrays(self):
        ...
        self.RES_LAYER_HEIGHT = np.full(
            (self.ny, self.nx, cn.max_layers), np.nan
        )
        self.RES_LAYER_RHO = np.full((self.ny, self.nx, cn.max_layers), np.nan)
        self.RES_LAYER_T = np.full((self.ny, self.nx, cn.max_layers), np.nan)
        self.RES_LAYER_LWC = np.full((self.ny, self.nx, cn.max_layers), np.nan)
        self.RES_LAYER_IF = np.full((self.ny, self.nx, cn.max_layers), np.nan)
    ...
    def write_restart_to_file(self):
        self.add_variable_along_latlon(
            self.RESTART, self.RES_NLAYERS, "NLAYERS", "-", "Number of layers"
        )
        self.add_variable_along_latlon(
            self.RESTART,
            self.RES_NEWSNOWHEIGHT,
            "new_snow_height",
            "m .w.e",
            "New snow height",
        )
        ...
        self.add_variable_along_latlonlayer(
            self.RESTART, self.RES_LAYER_T, "LAYER_T", "K", "Layer temperature"
        )
        self.add_variable_along_latlonlayer(
            self.RESTART,
            self.RES_LAYER_LWC,
    ...
    def add_variable_along_latlon(self, ds, var, name, units, long_name):
        """This function self.adds missing variables to the self.DATA class"""
        ds[name] = ((cfg.northing, cfg.easting), var.data)
        ds[name].attrs["units"] = units
        ds[name].attrs["long_name"] = long_name
        ds[name].encoding["_FillValue"] = -9999
        return ds
    ...
    def add_variable_along_latlontime(self, ds, var, name, units, long_name):
        """This function self.adds missing variables to the self.DATA class"""
        ds[name] = (("time", cfg.northing, cfg.easting), var.data)
        ds[name].attrs["units"] = units
        ds[name].attrs["long_name"] = long_name
        ds[name].encoding["_FillValue"] = -9999
        return ds
    ...
    def add_variable_along_latlonlayertime(
        self, ds, var, name, units, long_name
    ):
        """This function self.adds missing variables to the self.DATA class"""
        ds[name] = (("time", cfg.northing, cfg.easting, "layer"), var.data)
        ds[name].attrs["units"] = units
        ds[name].attrs["long_name"] = long_name
        ds[name].encoding["_FillValue"] = -9999
        return ds
    ...
    def add_variable_along_latlonlayer(self, ds, var, name, units, long_name):
        """This function self.adds missing variables to the self.DATA class"""
        ds[name] = ((cfg.northing, cfg.easting, "layer"), var.data)
        ds[name].attrs["units"] = units
        ds[name].attrs["long_name"] = long_name
        ds[name].encoding["_FillValue"] = -9999
        return ds
