...
import config
import constants
from cosipy.cpkernel.grid import _check_node_is_snow
...
def method_Boone(GRID, SLOPE, dt):
    """Densify snowpack through overburden pressure.

    Adapted from Essery et al. (2013).
    """

    height, rho, temperature, lwc, icf = copy_layer_profiles(GRID)

    if not config.use_debris:
        method_Boone_backend(
            GRID=GRID,
            dt=dt,
            rho=rho,
            height=height,
...
@njit(cache=False)
def method_Boone_backend_loop(
    GRID, M_s, rho, height, temperature, lwc, icf, dt, idxNode
):
    # Constants
    c1 = 2.8e-6
    c2 = 0.042
    ...
    eta0 = 3.7e7
    rho0 = 150
    if (rho[idxNode] < constants.snow_ice_threshold) & (
        height[idxNode] > constants.minimum_snow_layer_height
    ):
        ...
        # Viscosity
        eta = eta0 * np.exp(
            c4 * (constants.zero_temperature - temperature[idxNode])
            + c5 * rho[idxNode]
        )
        ...
        dRho = (
            ...
            + c1
            * np.exp(
                -c2 * (constants.zero_temperature - temperature[idxNode])
        ...
        GRID.set_node_liquid_water_content(
            idxNode, (1 + dtheta_w) * lwc[idxNode]
        )
        ...
        if (
            ...
            + GRID.get_node_liquid_water_content(idxNode)
            + GRID.get_node_porosity(idxNode)
            > 1.0
        ):
            print(
                (1 + dtheta_i) * GRID.get_node_ice_fraction(idxNode),
                (1 + dtheta_w) * GRID.get_node_liquid_water_content(idxNode),
                GRID.get_node_porosity(idxNode),
            )
            print(
                (1 + dtheta_i) * GRID.get_node_ice_fraction(idxNode)
...
@njit(cache=False)
def method_Boone_backend(GRID, dt, rho, height, temperature, lwc, icf):
    # Constants
    c1 = 2.8e-6
    c2 = 0.042
    c3 = 0.046
...
@njit(cache=False)
def method_Boone_backend_debris(GRID, dt, rho, height, temperature, lwc, icf):
    # Overburden snow mass
    M_s = 0.0
    ...
    idxNode = 0
    not_snow = 0
    while idxNode < GRID.get_number_snow_layers() + not_snow:
        if not _check_node_is_snow(GRID, idxNode):
            if idxNode > 0:
                M_s = M_s + rho[idxNode - 1] * height[idxNode - 1]
            elif idxNode == 0:
                M_s = M_s + rho[0] * (height[0] / 2.0)
    ...
    for idxNode in range(0, GRID.get_number_snow_layers(), 1):
        if (rho[idxNode] < constants.snow_ice_threshold) & (
            ...
            eta = (
                ...
                * eta0
                * (rho[idxNode] / c)
                * np.exp(a * (273.14 - t[idxNode]) + b * rho[idxNode])
            )
            ...
            if idxNode > 0:
                ...
                sigma = sigma + 9.81 * np.cos(SLOPE) * rho[0] * (
                    height[0] / 2.0
                )
    ...
    for idxNode in range(0, GRID.get_number_snow_layers(), 1):
        ...
        if (1 - (dRho / GRID.get_node_density(idxNode))) < 1:
            ...
            GRID.set_node_ice_fraction(
                idxNode,
                (rho_max + (rho[idxNode] - rho_max) * np.exp(-dt / tau))
            ...
            GRID.set_node_height(
                idxNode,
                (1 - (dRho / GRID.get_node_density(idxNode)))
