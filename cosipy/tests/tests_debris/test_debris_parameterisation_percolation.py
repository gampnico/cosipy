...
class TestParamPercolationDebris:
    ...
    #     for idx in range(grid_deb.number_nodes):
    #         if grid_deb.get_node_ntype(idx) == 1:
    #             assert grid_deb.get_node_liquid_water_content(idx) == 0.0
    #             assert grid_deb.get_node_irreducible_water_content(idx) == 0.0
    #             assert grid_deb.get_node_ice_fraction(idx) == 0.0
    ...
    def test_percolation_debris(
        ...
        conftest_debris_boilerplate.add_debris_to_grid(
            grid_obj=test_grid, debris=5, snow=arg_snow
        )
        heights = test_grid.get_height()
        lwc_array = np.full(test_grid.number_nodes, arg_lwc)
        test_grid.set_liquid_water_content(lwc_array)
        np.testing.assert_allclose(
            test_grid.get_liquid_water_content(), lwc_array
        )

        initial_mwe = (
            np.nansum(np.array(test_grid.get_liquid_water_content()) * heights)
            + arg_melt
        )

        runoff = module_percolation.percolation(
            test_grid, arg_melt, self.timedelta
        )
        ...
        assert all(i >= 0.0 for i in test_grid.get_liquid_water_content())
        # all liquid water exits last node
        last_node_lwc = test_grid.grid[-1].liquid_water_content
