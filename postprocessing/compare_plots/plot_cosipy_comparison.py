...
def get_correlation_coefficient(
    simulated: xr.DataArray, expected: xr.DataArray
) -> tuple:
    x_correlation = np.corrcoef(simulated[:, 0], expected[:, 0])[0, 1]
    y_correlation = np.corrcoef(simulated[:, 1], expected[:, 1])[0, 1]
    correlation = np.corrcoef(simulated.flatten(), expected.flatten())[0, 1]
    return x_correlation, y_correlation, correlation
    ...
    if short_name is None:  # stamp plots
        ...
        for i in range(len(variables)):
            for j, key in enumerate(variables[i]):
                axes[i][j] = plot_axes(
                    ax=axes[i][j],
                    array=obs_data[key]-model_data[key],
                    timestamp=timestamp,
                    mean=mean,
                    topography=model_data.HGT,
                    plot_type=plot_type,
                )
    else:
        ...
        axes = plot_axes(
            ax=axes,
            array=obs_data[short_name]-model_data[key],
            timestamp=timestamp,
            mean=mean,
            topography=model_data.HGT,
            plot_type=plot_type,
...
def parse_datasets(model_output: str, observations: str) -> tuple:
    data_model = xr.open_dataset(model_output)
    check_2d(data_model)
    print(data_model)
    data_obs = xr.open_dataset(observations)
    check_2d(data_obs)
    return data_model, data_obs
...
def plotContour(
    ...
    Args:
        model_output: COSIPY model output.
    ...
    x_corr, y_corr, corr = get_correlation_coefficient(
...
def parse_arguments() -> argparse.Namespace:
    ...
    Required arguments:
        ...
        -i, --input <path>      Path to COSIPY input observations.
    ...
    parser.add_argument(
        ...
        type=str,
        metavar="<path>",
        help="Path to COSIPY output .nc file",
    )
    parser.add_argument(
        "-i",
        "--input",
        ...
        type=str,
        metavar="<path>",
        help="Path to COSIPY input observations",
    )
    parser.add_argument(
        "-d",
        "--date",
...
def main():
    ...
    Produces cross-correlation field plots between input and outof single timesteps or daily
    mean for a single or all variables. Passing a date without ``--mean`` leads to an
    error.

    Required arguments:
        -f, --file <path>       Path to COSIPY output .nc file.
        -i, --input <path>      Path to COSIPY input observations.
        -d, --date <str>        Target date or timestamp.
